...
# the computing components of the workshop.
#
...
# of object XXX." We can get around this
...
with(VB, plot(Year,WinPct,
              ...
              col="navy", # make it blue, because Penn State
              main="PSU Women's Volleyball Winning\nPercentages, 2005-2021",
              xlab="Season",ylab="Winning Percentage")) # add axis labels
...
# the values of GOP (either 1 for Republican 
# appointees or 0 for Democratic ones) are on
# the horizontal one.
#
...
for(i in 1:NSims) {
  flips <- rbinom(N,1,0.5) # simulate N flips
  counts[i] <- sum(flips)  # store the count
}
